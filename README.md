...
| 子系统 | 状态 | 说明 |
| --- | --- | --- |
| FastAPI 后端 | `【已验证】` | 单方言重构后全部测试直接跑在 PostgreSQL 16 + pgvector：当前 `515 passed（2026-08-24，阿里云独立测试库）`；此前基线为 `508 passed (92.25s，2026-08-06 本机)`；原独立 PG 集成测试并入常规回归，不再有 skip |
...
| 管理员 AI 服务配置 | `【已验证：阿里云线上 + 隔离测试库】` | 仅 `admin` 可查看配置状态、保存生成/向量服务参数、执行付费连接测试并一键停用/启用；API Key 使用独立 Fernet 密钥加密入库，接口与审计均不返回明文。阿里云隔离 PostgreSQL 全量 `515 passed`；生产库已迁移至 `0018`，线上 RBAC、停用/恢复与 Edge 页面通过。当前公网仅 HTTP，前后端均禁止提交新 Key；真实 Key 页面保存和付费连接测试须在 HTTPS 后执行，见 `docs/evidence/admin_ai_config_deploy_20260824.md` |
