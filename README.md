...
| 子系统 | 状态 | 说明 |
| --- | --- | --- |
| FastAPI 后端 | `【已验证】` | 单方言重构后全部测试直接跑在 PostgreSQL 16 + pgvector：当前 `516 passed（2026-08-24，阿里云独立测试库）`；此前基线为 `508 passed (92.25s，2026-08-06 本机)`；原独立 PG 集成测试并入常规回归，不再有 skip |
...
| 管理员 AI 服务配置 | `【已验证：阿里云线上 + 隔离测试库】` | 仅 `admin` 可查看配置状态、保存生成/向量服务参数、执行付费连接测试并一键停用/启用；API Key 使用独立 Fernet 密钥加密入库，接口与审计均不返回明文。阿里云隔离 PostgreSQL 全量 `516 passed`；生产库已迁移至 `0018`，线上 RBAC、停用/恢复与 Edge 页面通过。当前公网仅 HTTP，前后端均禁止提交新 Key；真实 Key 页面保存和付费连接测试须在 HTTPS 后执行，见 `docs/evidence/admin_ai_config_deploy_20260824.md` |
...
API 启动时会检查数据库 revision；不在 Alembic `head` 时直接拒绝启动。当前 head 为 `20260824_0018`：`0005` 数据库状态约束，`0006` 独立 IP 登录桶与可信代理配套，`0007` API/Embedding 配额表，`0008` 生成层留痕表 generation_records，`0009` 内容缺口事件表 knowledge_gap_events，`0010` 会话表 conversations/conversation_messages，`0011` 诊断来源会话字段，`0012` 消息路由字段，`0013` 会话体验包（快捷操作、消息反馈），`0014` 知识库后台（文档生命周期、版本历史、缺口闭环），`0015` 生成记录的 token 用量与成本，`0016` 角色分级 viewer/operator，`0017` 会话知识缺口事件，`0018` 管理员 AI 服务加密配置；`create_all()` 仅保留给显式开启的隔离测试。
