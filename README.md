...
| 管理员 AI 服务配置 | `【已验证：隔离测试库，待线上部署】` | 仅 `admin` 可查看配置状态、保存生成/向量服务参数、执行付费连接测试并一键停用/启用；API Key 使用独立 Fernet 密钥加密入库，接口与审计均不返回明文。新增 5 条接口/RBAC/加密/热切换测试，并与 11 条迁移测试在阿里云 PostgreSQL 16 + pgvector 独立库共同通过；尚未在生产库迁移或操作现网开关 |
