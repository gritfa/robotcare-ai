...
判断边界（2026-08-04 专家终审纠偏，勿越界）：
- 分类（issue_category_code）判的是 query 与类目**语义**是否吻合，不要求说明书
  原文直接定义该类目名——说明书没写类目名不构成可疑
- 诊断流程是否已发布、步骤先后顺序由平台安全策略定义，说明书无需声明
  「这是第一步」——不要以此否决 step_selection/classification 用例
- 只有「页码指向内容与 query 无关」「断言与页原文矛盾」这类事实错误才算可疑
