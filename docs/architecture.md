...
- 【已验证】后端使用 FastAPI、Pydantic v2、SQLAlchemy 2、JWT、Argon2 与 Alembic；当前 head `20260720_0004` 保存诊断分类决策，本地备份升级和 metadata 零漂移已验证。
...
- 【已验证：本机关键链路】Playwright 已在真实 Microsoft Edge 上完成 5 条 E2E：原 3 条闭环、分类冲突改选和高风险持久卡片；PDF 校验落盘、非空和 `%PDF-`，命令 `38.4s` 正常退出。隔离 SQLite 不替代生产 PostgreSQL/Alembic/Docker/HTTPS 验收。
