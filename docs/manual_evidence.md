...
- 【已验证】：两份 PDF 可从海尔官方直链下载；官方下载内容与仓库文件的字节数和 SHA256 一致；文件可读取、可提取文本，且指定页面已渲染抽检。
...
| 型号 | 官方产品页 | 官方 PDF 直链 | 仓库文件 | 字节数 | SHA256 | 页数 | 可提取字符数 | 下载 | 文本提取 | 向量入库 |
| --- | --- | --- | --- | ---: | --- | ---: | ---: | --- | --- | --- |
...
- 说明书版权归海尔及相关权利人所有；仓库所有者已确认具备这两份文件的公开再分发授权。
- 公开授权范围内的 `JH69U1_manual.pdf`、`VC35U1_manual.pdf` 纳入 `knowledge/raw/`；该目录中的其他原始资料仍默认忽略。
- 仓库同时保留官方来源 URL、文件指纹、页数、提取统计和处理状态，便于核验文件来源与完整性。
